class PostsController < ApplicationController
  ...
  def index
    if params[:tagged]
      @posts = Post.scoped_posts(signed_in?).tagged_with(params[:tagged]).paginate(page: params[:page], per_page: 5)
    else
      @posts = Post.scoped_posts(signed_in?).paginate(page: params[:page], per_page: 5)
    end
  end
